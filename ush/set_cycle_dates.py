...
def set_cycle_dates(date_start, date_end, incr_cycl_freq):
    """
    Sets the cycle date(s). Given the start and end dates and a cycling frequency, returns an array of
    cycle date-hours whose elements have the form ``YYYYMMDDHH``, where ``YYYY`` is a four-digit year, ``MM`` is a two- digit month, ``DD`` is a

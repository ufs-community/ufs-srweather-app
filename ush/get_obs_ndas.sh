...
into 24 but doesn't:
  ...
  24 % NDAS_OBS_AVAIL_INTVL_HRS = ${remainder}"
fi
...
arcv_hr = ${arcv_hr}"
  ...
  # archive.
  arcv_contents_yyyymmddhh_start=$(${DATE_UTIL} --date "${yyyymmdd_arcv} ${hh_arcv} 6 hours ago" +%Y%m%d%H)
  arcv_contents_yyyymmddhh_end=$(${DATE_UTIL} --date "${yyyymmdd_arcv} ${hh_arcv} 1 hours ago" +%Y%m%d%H)
  do_retrieve="FALSE"
...
spanned by the current ${arcv_hr_incr}-hourly archive file.  The bounds of the current
archive are:
...
The times at which obs need to be retrieved are:
  obs_retrieve_times_crnt_day = ($(printf "\"%s\" " ${obs_retrieve_times_crnt_day[@]}))"
    ...
    # them in the process.
    #
    # Note that the tm06 file in a given archive contain more/better observations
    # than the tm00 file in the next archive (their valid times are equivalent),
    # so we use the tm06 files.
    for hrs_ago in $(seq --format="%02g" 6 -${NDAS_OBS_AVAIL_INTVL_HRS} 1); do

...
There is an example batch-submit script for hera (Slurm) and derecho (PBS).  These examples set the build and run environment for hera or derecho, so that run-time libraries match the compiled libraries (i.e. netcdf, mpi). 
...
6. Run each of the listed scripts, in the order given.  Scripts with the same stage-# may be run simultaneously.
 ...
 - Example scripts for Slurm (hera) and for PBS (derecho) are provided.  These will need to be adapted to your system

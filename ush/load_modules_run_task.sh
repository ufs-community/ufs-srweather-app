...
#
# Note that the build of NEMSfv3gfs generates the shell script at 
#
#   ${NEMSfv3gfs_DIR}/NEMS/src/conf/module-setup.sh
#
...
# it is only found in the NEMSfv3gfs code, here we prefer to perform our
# own initialization.  Ideally, there should be one module-setup.sh 
# script that is used by all external repos/codes, but such a script 
# does not exist.  If/when it does, we will consider switching to it in-
# stead of using the case-statement below.
#
#-----------------------------------------------------------------------
#
...
#
#   ${NEMSfv3gfs_DIR}/NEMS/src/conf/modules.fv3
#
# Here, NEMSfv3gfs_DIR is the directory in which the NEMSfv3gfs reposi-
# tory containing the FV3 model is cloned (normally "$HOMErrfs/sorc/
# NEMSfv3gfs"), and modules.fv3 is a module file that is generated by
# the NEMSfv3gfs build process.  It contains the appropriate modules to
# use when running the FV3 model.  Thus, we just point to it via the 
# symlink "run_fcst_ccpp" in the modulefiles/$machine directory.
#
# QUESTION:
...
# file in the NEMSfv3gfs external repo.  This latter target file will 
# exist only if the NEMSfv3gfs code has already been built.  Thus, we
# now check to make sure that the module file exits.
#
#-----------------------------------------------------------------------
#

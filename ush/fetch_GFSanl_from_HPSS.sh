...
module load hpss
#
# The following variables must be specified by the calling script.
# 
INIDIR=${INIDIR:?"INIDIR has not been set.  Exiting script."}
HPSS_DIR=${HPSS_DIR:?"HPSS_DIR has not been set.  Exiting script."}
TAR_FILE=${TAR_FILE:?"TAR_FILE has not been set.  Exiting script."}
#
...
# it exists but is emtpy, we try to fetch a new copy of the GFS analysis
# file from HPSS.
#
...
# then the ls command in the second set of square brackets in the if-
# statement below returns an emtpy string.  This causes the overall arg-
# ument of the if-statment to be false, which in turn causes the remain-
# der of this script to execute.
#
if [ -d "$INIDIR" ] && \
   [ "$(ls -A $INIDIR)" ]; then
  echo
...
tar -xvf $INIDIR/$TAR_FILE --directory=$INIDIR \
  ./pgrba/cmc_gespr.t00z.pgrbaf336 ./pgrba/cmc_gespr.t00z.pgrbaf342  # <-- Couple of dummy files to extract from the dummy tar file.

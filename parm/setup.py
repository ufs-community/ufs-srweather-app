...
def load_config_for_setup(parmsrw, default_config, user_config):
    ...
    Args:
      parmsrw            (str): Path to the parm directory for SRW
    ...
    machine_file = os.path.join(parmsrw, "machine", f"{lowercase(machine)}.yaml")
    ...
    cfg_f = load_config_file(
        os.path.join(parmsrw, "fixed_files_mapping.yaml")
    )

    # Load the constants file
    cfg_c = load_config_file(os.path.join(parmsrw, "constants.yaml"))

    # Load the rocoto workflow default file
    cfg_wflow = load_config_file(os.path.join(parmsrw, "wflow", "default_workflow.yaml"))
...
def set_srw_paths(parmsrw, expt_config):
    ...
    Args:
       parmsrw:     (str) path to the system location of the parm directory
                     under the SRW clone
...
def setup(PARMsrw, user_config_fn="config.yaml", debug: bool = False):
    ...
    Args:
      PARMsrw        (str) : The full path of the parm directory where
    ...
    # Create a dictionary of config options from defaults, machine, and user config files.
    default_config_fp = os.path.join(PARMsrw, "config_defaults.yaml")
    user_config_fp = os.path.join(PARMsrw, user_config_fn)
    expt_config = load_config_for_setup(PARMsrw, default_config_fp, user_config_fp)

    # Set up some paths relative to the SRW clone
    expt_config["user"].update(set_srw_paths(PARMsrw, expt_config))
    ...
    cfg_v = load_config_file(os.path.join(PARMsrw, "valid_param_vals.yaml"))
...
if __name__ == "__main__":
    PARMsrw = os.path.dirname(os.path.abspath(__file__))
    setup(PARMsrw)

...
. ${PARMsrw}/load_modules_wflow.sh ${machine}
...
#
    if [ "${called_from_cron}" = "TRUE" ]; then
       ${PARMsrw}/get_crontab_contents.py --remove -m=${machine} -l='${CRONTAB_LINE}' -c -d
    else
       ${PARMsrw}/get_crontab_contents.py --remove -m=${machine} -l='${CRONTAB_LINE}' -d
    fi
  fi
#

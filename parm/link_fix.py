...
def link_fix(
    ...
    **kwargs,
    ...
    # make_ic, make_lbc, and/or forecast tasks.
    #
    ...
    #    shaved down to obtain the filtered orography files with 3- and
    #    4-cell-wide halos.
    #
    ...
    #       "C*.grid.tile7.halo${NHW}.nc".
    #
    #    b) C*.grid.tile7.halo${NHW}.nc
    #       This is the
    ...
    #       but for clarity, it is then renamed "C*.raw_orog.tile7.halo${NHW}.nc".
    #
    ...
    #       "${CRES}.grid.tile7.halo${NHW}.nc").  The executable then
    ...
    #
    #    b) C*.raw_orog.tile7.halo${NHW}.nc
    ...
    #       "C*.filtered_orog.tile7.halo${NHW}.nc" that will be
    ...
    #    file, which should be "C*.grid.tile7.halo4.nc".  The
    #    dimensions specified in this grid file must match the ones specified
    #    in the (filtered) orography file "C*.oro_data.tile7.halo4.nc"
    ...
    #    exist:
    #    a) "C*.mosaic.halo4.nc"
    #    b) "C*.grid.tile7.halo4.nc"
    #    c) "C*.oro_data.tile7.halo4.nc"
    #
    # -----------------------------------------------------------------------
    #
    #
    if file_group == "grid":
        fns = [
            f"C*_mosaic.halo{nhw}.nc",
            f"C*_mosaic.halo4.nc",
            f"C*_mosaic.halo3.nc",
            f"C*_grid.tile7.halo{nhw}.nc",
            f"C*_grid.tile7.halo3.nc",
            f"C*_grid.tile7.halo4.nc",
        ]

    elif file_group == "orog":
        fns = [
            f"C*_oro_data.tile7.halo0.nc",
            f"C*_oro_data.tile7.halo4.nc",
        ]
        if ccpp_phys_suite == "FV3_RAP" or ccpp_phys_suite == "FV3_HRRR" or ccpp_phys_suite == "FV3_HRRR_gf" or ccpp_phys_suite == "FV3_GFS_v17_p8":
            fns += [
                f"C*_oro_data_ss.tile7.halo0.nc",
                f"C*_oro_data_ls.tile7.halo0.nc",
            ]
    ...
    #
        ...
        for sfc_climo_field in sfc_climo_fields:
            fns.append(f"C*.{sfc_climo_field}.tile7.halo0.nc")
            fns.append(f"C*.{sfc_climo_field}.tile7.halo4.nc")
    ...
    #
    if file_group == "grid":
        target = f"{cres}_grid.tile7.halo4.nc"
        symlink = f"{cres}_grid.tile7.nc"
    ...
    if file_group == "sfc_climo":
        ...
        for field in sfc_climo_fields:
            ...
            halo = f"{cres}.{field}.tile7.halo4.nc"
            no_halo = re.sub(f".halo4", "", halo)
            create_symlink_to_file(halo, no_halo, True)

            # Create links without halo and tile7, and with "tile1"
            halo_tile = f"{cres}.{field}.tile7.halo0.nc"
            no_halo_tile = re.sub(f"tile7.halo0", "tile1", halo_tile)

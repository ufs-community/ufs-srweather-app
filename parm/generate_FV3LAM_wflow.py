...
def generate_FV3LAM_wflow(
    ...
    Args:
        parmsrw (str) : The full path of the parm directory where this script is located
    ...
    expt_config = setup(parmsrw,debug=debug)
    ...
    if expt_config["platform"]["WORKFLOW_MANAGER"] == "rocoto":
        ...
        template_xml_fp = os.path.join(
            expt_config["user"]["HOMEdir"],"parm/templates",wflow_xml_fn,
        )
    ...
    # -----------------------------------------------------------------------
    #
    cp_vrfy(os.path.join(parmsrw, EXPT_CONFIG_FN), EXPTDIR)
...
if __name__ == "__main__":
    ...
    wflow_logfile = f"{PARMsrw}/log.generate_FV3LAM_wflow"

    # Call the generate_FV3LAM_wflow function defined above to generate the
    # experiment/workflow.
    try:
        expt_dir = generate_FV3LAM_wflow(PARMsrw, wflow_logfile, pargs.debug)

...
Here, ``SRC_HOST`` refers to the (HPC or cloud) system where the ``.png`` files are currently located. ``DEST_HOST`` refers to the user's local system. ``file1`` and ``file2`` could be directories instead of files. 

If an ssh tunnel is already established between the local system and a remote system, users can run one of the following commands (adjusting the username and file paths to match the user's systems):

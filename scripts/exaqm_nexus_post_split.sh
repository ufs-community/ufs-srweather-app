...
#-----------------------------------------------------------------------
#
cp ${PARMdir}/nexus_config/cmaq/HEMCO_sa_Time.rc ${DATA}/HEMCO_sa_Time.rc
...
if [ "${NUM_SPLIT_NEXUS}" = "01" ]; then
  ...
  ${USHdir}/nexus_utils/python/concatenate_nexus_post_split.py "${COMIN}/${cyc}/NEXUS/${NET}.${cycle}${dot_ensmem}.NEXUS_Expt_split.*.nc" "${DATA}/NEXUS_Expt_combined.nc"
  export err=$?
  if [ $err -ne 0 ]; then
...
#-----------------------------------------------------------------------
#
${USHdir}/nexus_utils/combine_ant_bio.py "${DATA}/NEXUS_Expt_combined.nc" ${DATA}/NEXUS_Expt.nc
export err=$?
if [ $err -ne 0 ]; then

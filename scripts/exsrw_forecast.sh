...
task_global_vars=( "AQM_RC_PRODUCT_FN" "CCPP_PHYS_DIR" "CCPP_PHYS_SUITE" \
  ...
  "OMP_NUM_THREADS_FORECAST" "PE_MEMBER01" "PRINT_ESMF" "PREDEF_GRID_NAME" \
  "PRE_TASK_CMDS" "POST_OUTPUT_DOMAIN_NAME" "RESTART_INTERVAL" \
  "RUN_CMD_FCST" "SMOKE_DUST_FILE_PREFIX" "SUB_HOURLY_POST" \
...
if ([ "$STOCH" == "TRUE" ] && [ $(boolify "${DO_ENSEMBLE}") = "TRUE" ]); then
  ${PARMsrw}/set_fv3nml_ens_stoch_seeds.py \

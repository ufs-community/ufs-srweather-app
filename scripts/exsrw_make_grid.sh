...
task_global_vars=( "DEL_ANGLE_X_SG" "DEL_ANGLE_Y_SG" "GFDLgrid_NUM_CELLS" \
  "GRID_DIR" "GRID_GEN_METHOD" "LAT_CTR" "LON_CTR" \
  "NEG_NX_OF_DOM_WITH_WIDE_HALO" "NEG_NY_OF_DOM_WITH_WIDE_HALO" "NHW" \
  "NX" "NY" "PAZI" "PRE_TASK_CMDS" "RES_IN_FIXLAM_FILENAMES" \
  "RGNL_GRID_NML_FN" "RUN_CMD_SERIAL" )
...
grid_fn="${CRES}_grid.tile7.halo${NHW}.nc"
...
for halo_num in "${halo_num_list[@]}"; do
  ...
  shaved_fp="${DATA}/${CRES}_grid.tile7.halo${halo_num}.nc"
  printf "%s %s %s %s %s\n" \
...
for halo_num in "${halo_num_list[@]}"; do
  ...
  grid_fn="${CRES}_grid.tile7.halo${halo_num}.nc"
  grid_fp="${GRID_DIR}/${grid_fn}"
  mosaic_fn="${CRES}_mosaic.halo${halo_num}.nc"
